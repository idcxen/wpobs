<?php
/**
Plugin Name: WPOBS华为云对象存储插件
Plugin URI: https://www.lezaiyun.com/wpobs.html
Description: WordPress同步附件内容远程至华为Obs对象存储中，实现网站数据与静态资源分离，提高网站加载速度。公众号：  <font color="red">乐在云</font>
Version: 2.1
Author: 老蒋和他的小伙伴
...
if (!class_exists('WPObs')) {
    class WPObs {

        private $option_name     = 'WPObs_options';               // 插件参数保存名称
        private $menu_title      = '华为云对象存储设置';            // 设置菜单的菜单名
        private $page_title      = '华为云对象存储设置';            // 设置菜单的页面title
        private $capability      = 'manage_options';              // 设置页面管理所需权限
        private $version         = '2.1';                         // 插件数据版本， 每次修改应与上方的Version值相同
        private $setting_notices = [
            'update_success' => '插件设置保存完毕',               // post数据保存成功时提示内容
            'update_failed'  => '插件设置更新失败',               // 失败时提示
        ];

        private $image_display_default_value = 'x-image-process=image/format,webp/sharpen,100/blur,r_1,s_1';  // 图片处理服务默认规则
        private $image_display_default_tab   = '?';                                               // 图片处理服务 url连字符
        ...
        private function constants() {
            ...
            $this->object_storage = new ObsStorageObjectApi($this->options);  // option更新后，若变动了参数，则Api实例重新创建，目前只有setting中会触发
        }
        ...
        public function delete_remote_attachment($post_id) {
            ...
            if ( !empty( $deleteObjects ) ) {
                // 执行删除远程对象  wpobs批量删除格式较为独特，这里还是一个个删吧。
//                $allKeys = array_chunk($deleteObjects, 1000);  # 每次最多删除1000个，多于1000循环进行
                foreach ($deleteObjects as $keys){
                    //删除文件
        ...
        private function legacy_data_replace() {
            ...
            $this->options['opt']['legacy_data_replace'] = 1;
            update_option($this->option_name, $this->options);  // 文字替换，参数变动不影响QiNiuApi实例
            return $this->options;
        }
        ...
        public function setting_page() {
            ...
            if ($this->options && isset($_GET['_wpnonce']) && wp_verify_nonce($_GET['_wpnonce']) && !empty($_POST)) {
                if($_POST['type'] == 'info_set') {
                    ...
                    $this->options['obs_key'] = isset($_POST['obs_key']) ? sanitize_text_field(trim(stripslashes($_POST['obs_key']))) : '';
                    $this->options['obs_secret'] = isset($_POST['obs_secret']) ? sanitize_text_field(trim(stripslashes($_POST['obs_secret']))) : '';
                    ...
                    $this->object_storage = new ObsStorageObjectApi($this->options);
